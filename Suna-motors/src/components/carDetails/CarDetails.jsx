...
function CarDetails() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-16 w-16 border-t-4 border-red-600"></div></div>;
  if (error || !car) return <div className="min-h-screen flex flex-col items-center justify-center p-6"><h2 className="text-2xl font-bold text-red-600 mb-4">Error: {error}</h2><Link to="/cars" className="bg-red-600 text-white px-8 py-3 rounded-full">Back to Inventory</Link></div>;
            ...
            <div className="space-y-6">
                    ...
                    {images.map((img, index) => (
                      <button
                        ...
                        className={`relative flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-4 transition-all ${
                          activeImageIndex === index ? 'border-red-600 scale-95' : 'border-transparent opacity-60 hover:opacity-100'
                        }`}
                      >
              ...
              <div className="bg-white rounded-3xl p-8 shadow-xl">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h1 className="text-4xl font-black">{car.year} {car.brand} {car.model}</h1>
                    <p className="text-gray-500 mt-2">Ref ID: {car._id.slice(-8).toUpperCase()}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Price</p>
                    <p className="text-3xl font-black text-red-600">KES {Number(car.price).toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-8 border-y border-gray-100 mb-8">
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-red-600">speed</span>
                    <div><p className="text-xs text-gray-500 uppercase">Mileage</p><p className="font-bold">{car.mileage?.toLocaleString()} km</p></div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-red-600">local_gas_station</span>
                    <div><p className="text-xs text-gray-500 uppercase">Fuel</p><p className="font-bold">{car.fuelType}</p></div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-red-600">settings</span>
                    <div><p className="text-xs text-gray-500 uppercase">Gearbox</p><p className="font-bold">{car.transmission}</p></div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-red-600">palette</span>
                    <div><p className="text-xs text-gray-500 uppercase">Color</p><p className="font-bold">{car.color || 'Unspecified'}</p></div>
                  </div>
                </div>
            ...
            <div className="lg:sticky lg:top-8 h-fit space-y-6">
              <div className="bg-white rounded-3xl p-8 shadow-2xl border border-gray-100">
                ...
                <div className="space-y-4">
                  ...
                  <a href="tel:0792669697" className="w-full py-4 bg-red-600 text-white font-bold rounded-2xl hover:bg-red-700 transition flex items-center justify-center gap-2">
                  ...
                  <div className="bg-gray-50 p-4 rounded-2xl">
                    ...
                    <p className="text-xs text-gray-600">Pay as low as <span className="font-bold text-gray-900">KES {Math.round(car.price * 0.025).toLocaleString()}/mo</span> with 30% deposit.</p>
                  </div>
                </div>
              </div>
            </div>
